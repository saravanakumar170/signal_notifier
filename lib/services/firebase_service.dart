...
class FirebaseService {
  ...
  void _processSignal(SignalModel signal) {
    // Skip if this is the same signal we just processed
    if (_lastProcessedSignal == signal.type && 
        _lastSignalTime != null &&
        signal.timestamp.difference(_lastSignalTime!).inSeconds < 5) {
      debugPrint('Skipping duplicate signal: ${signal.type}');
      return;
    }
    ...
    _signalManager.sendSignal(signal.type);
  }
